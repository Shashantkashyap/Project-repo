...
function Assessment() {
  ...
  const handleFinalSubmit = async () => {
    try {
      ...
      const sso_id = decryptData(
        localStorage.getItem("sso_id"),
        import.meta.env.VITE_SECRET_KEY
      );
      const exam_name = decryptData(
      ...
      localStorage.clear();
       ...
       if (res.status === 400) {
        ...
        navigate("/error?reason=User already submit the responses");
        localStorage.clear();
      }
        ...
        navigate("/error?reason=assessment-submit-failed");
        localStorage.clear();
      }
      ...
      setError(
        `Failed to submit assessment: ${err.response.data || err.message}`
      );
      navigate("/error?reason=assessment-submit-failed");
      localStorage.clear();
    } finally {
  ...
  useEffect(() => {
    if (isSubmitted) {
      if (countdown === 0) {
        window.location.href = `http://10.70.230.201/CandidateLogin.aspx`;
        return;
      }
      ...
      <div className="max-w-4xl mx-auto mt-10">
        {isSubmitted ? (
          ...
          >
            ...
            <p>Please log in again to finish the rest of the assessment.</p>
          ...
          <>
           <div className="py-5 mt-15 mb-5 rounded-2xl w-full flex justify-center items-center">
  {!isAuthenticated && (
    <div className="flex flex-wrap gap-4 justify-center">
      {/* Name Box */}
      <div className=" rounded-xl p-3 min-w-[150px] bg-white shadow-sm">
        <div className="text-sm sm:text-base font-semibold text-blue-700">Name:</div>
        <div className="text-gray-800">
          {decryptData(name, import.meta.env.VITE_SECRET_KEY) || (
      ...
      {/* Roll No Box */}
      <div className=" rounded-xl p-3 min-w-[150px] bg-white shadow-sm">
        <div className="text-sm sm:text-base font-semibold text-blue-700">Roll No.:</div>
        <div className="text-gray-800">
          {decryptData(roll_no, import.meta.env.VITE_SECRET_KEY) || (
            <span className="italic text-gray-400">N/A</span>
          )}
        </div>
      </div>
      ...
      {/* SSO-ID Box */}
      <div className=" rounded-xl p-3 min-w-[150px] bg-white shadow-sm">
        <div className="text-sm sm:text-base font-semibold text-blue-700">SSO-ID:</div>
        <div className="text-gray-800">
          {decryptData(sso_id, import.meta.env.VITE_SECRET_KEY) || (
            <span className="italic text-gray-400">N/A</span>
          )}
        </div>
      </div>
      ...
      {/* Exam Box */}
      <div className=" rounded-xl p-3 min-w-[150px] bg-white shadow-sm">
        <div className="text-sm sm:text-base font-semibold text-blue-700">EXAM:</div>
        <div className="text-gray-800">
          {decryptData(exam_name, import.meta.env.VITE_SECRET_KEY) || (
            <span className="italic text-gray-400">N/A</span>
          )}
        </div>
      </div>
