    ...
    const fetchAssessments = async () => {
      try {
        ...
        if (!submissionsData || !Array.isArray(submissionsData)) {
          throw new Error(
            "Decrypted submissions data is not in the expected format"
          );
        }
        ...
        setStatistics({
          ...
          completed: submissionsData.filter((a) => a.completionRate === 100)
            .length,
          inProgress: submissionsData.filter((a) => a.completionRate < 100)
            .length,
        });
      } catch (err) {
