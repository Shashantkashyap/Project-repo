...
// Utility function to sanitize input - only alphanumeric, spaces, ?, -, and .
...
function EditQuestionModal({ open, question, onClose, onSave }) {
  ...
  useEffect(() => {
    if (question) {
      ...
      setError('');
      setExam(question.exam_name || '');
      setSectionId(question.section_id ? question.section_id.toString() : '');
      setOriginalSectionId(question.section_id ? question.section_id.toString() : '');
      setEditSectionMode(false);
    }
  }, [question]);

  // Fetch exams when entering edit section mode
  useEffect(() => {
    if (editSectionMode) {
      setLoadingExams(true);
      setExamApiError('');
      axios.get('http://13.201.44.91:3000/api/v1/admin/get-exam-names'
        , {
  ...
  // Fetch sections when exam changes in edit mode
  useEffect(() => {
    if (editSectionMode && exam) {
      ...
      axios.post(
        ...
        .then(res => {
          if (res.data && Array.isArray(res.data.sections)) {
            ...
            // If original section exists in new exam, select it, else first
            if (originalSectionId && res.data.sections.some(s => s.id.toString() === originalSectionId)) {
              setSectionId(originalSectionId);
            } else if (res.data.sections.length > 0) {
  ...
  }, [editSectionMode, exam, originalSectionId]);

  const handleOptionChange = (idx, field, value) => {
    // Sanitize option text input
    const sanitizedValue = field === 'option_text' ? sanitizeInput(value) : value;
    
    setOptions(opts =>
      opts.map((opt, i) =>
        i === idx ? { ...opt, [field]: sanitizedValue } : opt
      )
    );
  };
  ...
  const handleSave = async () => {
    ...
    setError('');
    try {
      let payloadSectionId = originalSectionId;
      let payloadExam = exam;
      if (editSectionMode) {
        // If user changed section, use new, else 0
        payloadSectionId = (sectionId !== originalSectionId) ? sectionId : '0';
        payloadExam = exam;
      }
      
      // Sanitize question text before saving
      const sanitizedQuestionText = sanitizeInput(questionText);
      
      const updatedQuestion = {
        question_id: question.question_id,
        question_text: sanitizedQuestionText,
        section_id: payloadSectionId,
        exam_name: payloadExam,
        options: options.map(opt => ({
          ...opt,
          option_text: sanitizeInput(opt.option_text),
      ...
      {!closing && (
        ...
        >
          ...
          >
            ...
            <div className="p-10" style={{ maxHeight: '90vh', overflowY: 'auto' }}>
              <div className="text-center font-bold text-blue-700 text-lg uppercase tracking-wider mb-6">
                Edit Question
              </div>
              {/* Section: show as text, edit on demand */}
              <div className="mb-6">
                ...
                {editSectionMode ? (
                  <>
                    ...
                    <div className="flex gap-2 mt-2">
                      <button
                        ...
                        onClick={() => { setEditSectionMode(false); setExam(question.exam_name || ''); setSectionId(originalSectionId); }}
                        disabled={saving}
                      >
                        Cancel
                      </button>
                  ...
                  <div className="flex gap-2 items-center">
                    <span className="px-4 py-2 rounded bg-gray-100 border border-gray-200 text-gray-800">
                      {question.section_name || 'N/A'}
                    </span>
                    <button
                      type="button"
                      className="px-3 py-1 rounded bg-blue-100 text-blue-700 font-semibold hover:bg-blue-200 transition"
                      onClick={() => setEditSectionMode(true)}
                    >
                      Edit Section
                    </button>
                  </div>
              ...
              <div className="mb-6">
                ...
                <input
                  ...
                  value={questionText}
                  onChange={e => setQuestionText(sanitizeInput(e.target.value))}
              ...
              <div>
                <div className="text-gray-700 font-semibold mb-3 flex items-center justify-between">
                  ...
                  <button
                    ...
                    disabled={options.length >= 5 || saving}
                    title="Add Option"
                    type="button"
                  >
                    +
...
function AddSectionModal({ open, onClose, onAdd }) {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      const sanitizedSectionName = sanitizeInput(sectionName.trim());
      ...
      if (res.data && res.data.message) {
        ...
        onAdd(`${exam} - ${sanitizedSectionName}${sanitizedDescription ? ` (${sanitizedDescription})` : ''}`);
        onClose();
      } else {
            ...
            <div className="p-8">
              ...
              <div className="mb-4">
                ...
                <input
                  ...
                  value={sectionName}
                  onChange={e => setSectionName(sanitizeInput(e.target.value))}
                  disabled={adding}
                />
              </div>
              <div className="mb-4">
...
function AddQuestionModal({ open, onClose, onAdd }) {
  ...
  const [exam, setExam] = useState(''); // <-- selected exam
  const [sections, setSections] = useState([]);
  const [sectionId, setSectionId] = useState('');
  const [questionText, setQuestionText] = useState('');
  ...
  useEffect(() => {
    if (exam) {
      ...
      setSectionId('');
      const token = localStorage.getItem('adminToken');
      axios.post(`${import.meta.env.VITE_API_BASE_URL}/admin/get-sections`, { exam_name: exam }, {
        headers: {
          'Authorization': `Bearer ${token}`
        },
        withCredentials: true // Ensure cookies are sent with the request
  ...
  const handleAdd = async () => {
    ...
    const questionPayload = {
      ...
      options: options.map(opt => ({
        ...opt,
        option_text: sanitizeInput(opt.option_text.trim()),
        rating: Number(opt.rating) || 0
      }))
    };
    try {
        ...
        >
          ...
          >
            ...
            <div className="p-10">
              ...
              {/* Section Dropdown (only if exam selected) */}
              {exam && (
                <div className="mb-6">
                  <label className="block text-gray-700 font-semibold mb-2">Section</label>
                  {loadingSections ? (
...
function QuestionsPage() {
  ...
  const [loadingFilterExams, setLoadingFilterExams] = useState(false);
  const [loadingFilterSections, setLoadingFilterSections] = useState(false);

  // Fetch exams for filter dropdown on mount
  useEffect(() => {
    setLoadingFilterExams(true);
    axios.get(`${import.meta.env.VITE_API_BASE_URL}/admin/get-exam-names`
      , {
        withCredentials: true,
      }
      ...
      .then(res => {
        ...
        if (rows && Array.isArray(rows)) {
          setFilterExams(rows.map(row => row.exam_name).filter(Boolean));
        } else {
          setFilterExams([]);
        }
      })
      .catch(() => setFilterExams([]))
      .finally(() => setLoadingFilterExams(false));
  }, []);

  // Fetch sections for selected exam in filter
  useEffect(() => {
    if (filterExam) {
      ...
      setFilterSection('');
      const token = localStorage.getItem('adminToken');
      axios.post(`${import.meta.env.VITE_API_BASE_URL}/admin/get-sections`, { exam_name: filterExam }, {
        headers: { 'Authorization': `Bearer ${token}` },
        withCredentials: true // Ensure cookies are sent with the request
      })
        .then(res => {
          if (res.data && Array.isArray(res.data.sections)) {
            setFilterSections(res.data.sections);
          } else {
            setFilterSections([]);
          }
        })
  ...
  const filteredQuestions = questions.filter(q => {
    if (filterExam && filterSection) {
      return q.section_id?.toString() === filterSection;
    } else if (filterExam) {
      // Show all questions for selected exam (by matching section's exam_name)
      const section = filterSections.find(s => s.id.toString() === q.section_id?.toString());
      return section && section.exam_name === filterExam;
    } else {
      return true;
    }
  });
    ...
    <div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
       ...
       <div className="flex mb-4 justify-end items-center gap-3">
          ...
          <button
            className={`flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg font-medium shadow-sm hover:bg-blue-700 transition-all duration-150 focus:outline-none focus:ring-2 focus:ring-blue-400 ${(filterExam && filterSections.length > 0) ? 'cursor-pointer' : 'opacity-50 cursor-not-allowed'}`}
            onClick={() => setAddQuestionOpen(true)}
            disabled={!(filterExam && filterSections.length > 0)}
            title={!(filterExam && filterSections.length > 0) ? "Select an exam with at least one section to add a question" : ""}
          >
          ...
          <div className="flex items-center ml-4 gap-2">
            <label className="text-gray-700 text-sm">Exam:</label>
            <select
              value={filterExam}
              ...
              disabled={loadingFilterExams}
            >
              <option value="">All Exams</option>
              {filterExams.map(name => (
                <option key={name} value={name}>{name}</option>
              ))}
            </select>
            <label className="text-gray-700 text-sm">Section:</label>
            <select
              value={filterSection}
              onChange={e => setFilterSection(e.target.value)}
              className="border border-gray-300 rounded-md px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white text-gray-800"
              disabled={loadingFilterSections || !filterExam}
            >
