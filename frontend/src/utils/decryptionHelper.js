...
export function decryptData(encryptedData, secretKey) {
  ...
  console.log('Attempting to decrypt data with multiple methods...' , secretKey);
...
/**
 * Decrypts AES-encrypted data using the default CryptoJS method (ECB, no IV, PKCS7).
 * @param {string} encryptedData - The encrypted string
 * @param {string} secretKey - The secret key
 * @returns {object|null} - The decrypted object or null if failed
 */
export function decryptDefaultAES(encryptedData, secretKey) {
  try {
    const bytes = CryptoJS.AES.decrypt(encryptedData, secretKey);
    const decryptedText = bytes.toString(CryptoJS.enc.Utf8);
    return JSON.parse(decryptedText);
  } catch (e) {
    console.error('Default AES decryption failed:', e.message);
    return null;
  }
}
